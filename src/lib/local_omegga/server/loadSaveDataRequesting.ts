...
export async function bakeSaveData(saveData: og.WriteSaveObject): Promise<string> {
    const saveFile = _tempSavePrefix + Date.now() + "_" + _tempCounter.save++;
...
export async function loadBricks(
    saveFile: string,
    { offX = 0, offY = 0, offZ = 0, quiet = false, correctPalette = false, correctCustom = false } = {}
) {
    // wait for the server to finish reading the save
    await Omegga.watchLogChunk(
        `Bricks.Load "${saveFile}" ${offX} ${offY} ${offZ} ${quiet ? 1 : 0} ${correctPalette ? 1 : 0} ${correctCustom ? 1 : 0}`,
        /^LogBrickSerializer: (.+)$/,
        {
...
export async function removeSaveFile(saveFile: string) {

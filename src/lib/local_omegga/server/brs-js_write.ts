...
import { DEFAULT_UUID, LATEST_VERSION, MAGIC, MAX_INT } from './brs-js_constants';
import { concat, isEqual, write } from './brs-js_utils';
import { WriteOptions } from './brs-js_types';

const EMPTY_ARR = new Uint8Array([]);

export const DEFAULT_COMPONENTS: og.DefinedComponents = {
    BCD_SpotLight: {
      version: 1,
      properties: {
        Rotation: 'Rotator',
...
export default function writeBrs(
    ...
    // compression is disabled by default // note: I am forcing no compression here
    const compress = write.uncompressed;
    ...
    const buff = concat(
      ...
      compress(
        ...
        write.array(
          ...
          save.brick_owners ?? [],
          ({ id = DEFAULT_UUID, name = 'Unknown', bricks = 0 }: Partial<og.Owner>) =>
      ...
      compress(
        write
          .bits()
          .each(save.bricks, function (brick: og.Brick, i) {
